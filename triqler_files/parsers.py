...
TriqlerInputRowHeaders = "run condition charge spectrumId linkPEP featureClusterId searchScore intensity PEP peptide proteins".split(" ")
TriqlerPEPInputRowHeaders = "run condition charge PEP intensity peptide proteins".split(" ")
  ...
  for i, row in enumerate(reader):
    ...
    if intensity > 0.0:
      if hasLinkPEPs:
        ...
        yield TriqlerInputRow(row[0], row[1], int(row[2]), int(row[3]), float(row[4]), int(row[5]), float(row[6]), intensity, 0, row[8], proteins)
      elif hasPEP:
        key = (int(row[2]), row[5])
        if key not in seenPeptChargePairs:
          seenPeptChargePairs[key] = len(seenPeptChargePairs)
        proteins = getUniqueProteins(row[6:])
        yield TriqlerInputRow(row[0], row[1], int(row[2]), (i+1) * 100, 0.0, seenPeptChargePairs[key], 0, intensity, float(row[3]), row[5], proteins)
      else:
        key = (int(row[2]), row[5])
        if key not in seenPeptChargePairs:
          seenPeptChargePairs[key] = len(seenPeptChargePairs)
        proteins = getUniqueProteins(row[6:])
        yield TriqlerInputRow(row[0], row[1], int(row[2]), (i+1) * 100, 0.0, seenPeptChargePairs[key], float(row[3]), intensity, 0, row[5], proteins)
...
def hasPEP(triqlerInputFile):
  reader = getTsvReader(triqlerInputFile)
  headers = next(reader)
  return "PEP" in headers
